...
import com.lpm.fanger.jdbc.bean.PageBean;
import com.lpm.fanger.jdbc.dao.BaseDao;
import com.lpm.fanger.jdbc.exception.DaoException;
import com.lpm.fanger.jdbc.utils.BeanUtilsBean;
...
public abstract class BaseDaoMysqlImpl<T, ID extends Serializable> extends
	...
	public List<T> search(String sql, List<Object> values,RowMapper<T> rowMapper){
		if (StringUtils.isEmpty(sql))
			return new ArrayList<T>();
		if (values == null)
			values = new ArrayList<Object>();
		logger.info("sql : " + sql + " values:" + values);
		List<T> list = this.getJdbcTemplate().query(sql, values.toArray(),rowMapper);
		return list == null ? new ArrayList<T>() : list;
	}
